...
// environment variables. If the environment variables don;t exist then continue except the case of CLD_S3_BUCKET where
// it will fail as no test can be done without it while in the other cases the EC2 metadat endpoint or program defaults may be sufficient
func getAndSetAwsS3ConfigFromEnv(srvCfg *shared.RuntimeConfig, t *testing.T, newStoreName string) {
	...
	srvCfg.Config.Backup[0].Target[0].Type = "aws_s3"
	...
	aKey := os.Getenv("CLD_AWS_ACCESS_KEY_ID")
	if aKey != "" {
	...
	aSecret := os.Getenv("CLD_AWS_SECRET_ACCESS_KEY")
	if aSecret != "" {
		srvCfg.Config.Backup[0].Target[0].Parameters = append(srvCfg.Config.Backup[0].Target[0].Parameters,
			shared.ConfigBackupTargetParams{
	...
	aBucketRegion := os.Getenv("CLD_S3_REGION")
	if aBucketRegion != "" {
		srvCfg.Config.Backup[0].Target[0].Parameters = append(srvCfg.Config.Backup[0].Target[0].Parameters,
			shared.ConfigBackupTargetParams{
				Name:  "region",
...
func TestAwsS3ValidateUploadDelete1(t *testing.T) {
	...
	getAndSetAwsS3ConfigFromEnv(result, t, targetName)
	...
	for _, store := range objectStores {
		...
		if storeName == targetName && StoreType == "aws_s3" {
			foundMatch = true
			objStore = store
			_, err = store.Validate()
			if err != nil {
				t.Fatalf("While validating the S3 object store, encountered error: %s", err)
			}
		}
	}
	if !foundMatch {
		t.Fatalf("Did not find any backup target matching expected type 'aws_s3' and name '%s'", targetName)
	}
