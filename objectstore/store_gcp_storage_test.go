...
// TODO - test all in S3 tests + crc32c ??

// wipes the "Parameters" of the first target belonging to the first backup and sets new parameter values based on
// environment variables. If the environment variables don;t exist then continue except the case of CLD_GCP_STORAGE_BUCKET where
// it will fail as no test can be done without it while in the other cases the GCP metadata endpoint or program defaults may be sufficient
func getAndSetGcpStorageConfigFromEnv(srvCfg *shared.RuntimeConfig, t *testing.T, newStoreName string) {
	...
	// force target type to be S3
	srvCfg.Config.Backup[0].Target[0].Type = "gcp_storage"
	...
	//
	optionalEnvVars := []string{"CLD_GCP_TYPE", "CLD_GCP_PROJECT_ID", "CLD_GCP_PRIVATE_KEY_ID", "CLD_GCP_PRIVATE_KEY",
		"CLD_GCP_CLIENT_EMAIL", "CLD_GCP_CLIENT_ID", "CLD_GCP_AUTH_URI", "CLD_GCP_TOKEN_URI",
		"CLD_GCP_AUTH_PROVIDER_X509_CERT_URL", "CLD_GCP_CLIENT_X509_CERT_URL"}
	for _, envVar := range optionalEnvVars {
		anEnvVar := os.Getenv(envVar)
		if anEnvVar != "" {
			if envVar == "CLD_GCP_PRIVATE_KEY" {
				// replace "\n" in the environment variable's contents with an actual new line instead of the characters "\" and "n"
				anEnvVar = strings.ReplaceAll(anEnvVar, `\n`, "\n")
			}
			srvCfg.Config.Backup[0].Target[0].Parameters = append(srvCfg.Config.Backup[0].Target[0].Parameters,
				shared.ConfigBackupTargetParams{
					Name:  strings.ToLower(envVar[8:]), // strip "CLD_GCP_" from variable name and then lower case the remainder
					Value: anEnvVar,
				},
			)
		}
...
func TestGCPValidateUploadDelete1(t *testing.T) {
	const targetName string = "gcp_1"
	...
	getAndSetGcpStorageConfigFromEnv(result, t, targetName)
	...
	for _, store := range objectStores {
		...
		if storeName == targetName && StoreType == "gcp_storage" {
			foundMatch = true
			objStore = store
			_, err = store.Validate()
			if err != nil {
				t.Fatalf("While validating the GCP object store, encountered error: %s", err)
			}
		}
	}
	if !foundMatch {
		t.Fatalf("Did not find any backup target matching expected type 'gcp_storage' and name '%s'", targetName)
	}
	...
	path, err := utils.SetupTmpFileWithContent([]byte(fileContent), "cloudbackup_TestGCPValidateUploadDelete1_sample_file_")
	if err != nil {
	...
	if strings.HasSuffix(err.Error(), gcpStorage.ErrObjectNotExist.Error()) {
...
func TestGCPValidateUploadDelete2(t *testing.T) {
	const targetName string = "gcp_1"
	cfgpath, pathsToDelete := testutils.SetupMockConfigAndTmpPaths(t, "unittest_backup_")
	// remove tmpfile which holds the yaml as the config has been parsed and loaded
	defer testutils.DeleteTestFilesAndDirs(pathsToDelete)
