...
class TestRestAPIReportNotification3(unittest.TestCase):
    def setUp(self):
        ...
        self.server_config_file_path, self.to_delete = setup_tmp_config_file_and_tmp_dirs(
            suffix='_integration_tests_rest_api_report_notification3_', config_file_content=self.complete_server_cfg)
        # tmp files for tests
    ...
    def test_notification_for_cancelled_backup(self):
        ...
        # wait for backup to show up as running
        counter = 0
        while True:
            # fetch again list of jobs and check that status of job, until it is running
            url = self.base_url + self.api_root + '/backup/list'
            r = requests.get(url=url, auth=(self.username, self.password))
            self.assertEqual(r.status_code, 200, url + " " + r.text)
            response = self.ValidatedAndDecodeResponse(r, url)
            ...
            if is_running:
                ...
                if counter > 50:
                    self.fail("Backup did not start running in 5 seconds")
                time.sleep(0.1)
                counter += 1
        ...
        # wait for backup to show up as stopped
        counter = 0
        while True:
        ...
        try:
            matches, email_msg = self.smtp_handler.received_message_matching(".*backup job \"{}\" has been "
                                                                             "cancelled.*".format(job_name))
        except UnicodeDecodeError:
        ...
        try:
            matches, email_msg = self.smtp_handler.received_message_matching(".*backup job \"{}\" having id {} has been "
                                                                             "cancelled.*".format(job_name, job_id))
        except UnicodeDecodeError:
...
def main():
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(TestRestAPIReportNotification3)
