...
class TestObjectStoreGcpStorage(unittest.TestCase):
    def setUp(self):
        ...
        self.server_config_file_path, self.to_delete = setup_tmp_config_file_and_tmp_dirs(
            suffix='_integration_tests_object_store_gcp_storage_')
        # client - config file
    ...
    def test_cmd_client_backup_target_test1(self):
        ...
        response['result']['backup'][job_index]['target'][0]['parameters'] = [
            ...
            {"name": "client_x509_cert_url",
             "value": "https://www.googleapis.com/robot/v1/metadata/x509/storage-tests%40badBADbadID.iam.gserviceaccount.com"},
        ]
    ...
    def test_full_backup(self):
        ...
        # if the private key env var is set then assume all of the credential key+values are set
        if settings["CLD_GCP_PRIVATE_KEY"]:
            response['result']['backup'][job_index]['target'][0]['parameters'] = [
                {"name": "type",
                 "value": settings["CLD_GCP_TYPE"]},
                {"name": "project_id",
                 "value": settings["CLD_GCP_PROJECT_ID"]},
                {"name": "private_key_id",
                 "value": settings["CLD_GCP_PRIVATE_KEY_ID"]},
                {"name": "private_key",
                 "value": settings["CLD_GCP_PRIVATE_KEY"]},
                {"name": "client_email",
                 "value": settings["CLD_GCP_CLIENT_EMAIL"]},
                {"name": "client_id",
                 "value": settings["CLD_GCP_CLIENT_ID"]},
                {"name": "auth_uri",
                 "value": settings["CLD_GCP_AUTH_URI"]},
                {"name": "token_uri",
                 "value": settings["CLD_GCP_TOKEN_URI"]},
                {"name": "auth_provider_x509_cert_url",
                 "value": settings["CLD_GCP_AUTH_PROVIDER_X509_CERT_URL"]},
                {"name": "client_x509_cert_url",
                 "value": settings["CLD_GCP_CLIENT_X509_CERT_URL"]},
            ]
        ...
        for path in response['result']['backup'][job_index]['paths']:
            tmp_expected_num_files, tmp_expected_num_dirs, tmp_expected_num_symlinks = count_files_folders_links(
                path, response['result']['backup'][job_index]["dereference"])
        ...
        logging.info("Starting backup with destination GCP Storage: {}/{}".format(bucket, prefix))
        req = {"name": job_name}
...
def main():
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(TestObjectStoreGcpStorage)
