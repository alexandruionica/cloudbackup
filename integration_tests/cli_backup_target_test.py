...
class TestBackupTargetTest(unittest.TestCase):
    def setUp(self):
        ...
        self.server_config_file_path, self.to_delete = setup_tmp_config_file_and_tmp_dirs(
            suffix='_integration_tests_cli_backup_target_test_')
        # client - config file
    ...
    def test_cmd_client_backup_target_test1(self):
        result = run_shell_cmd(self.cmd + " client backup target test first_backup -c " + self.client_config_file_path)
        self.assertEqual(result['result'].returncode, 0, "Exit code from {} is not 0. Command output object: "
                                                         "{}".format(cmd_default, result))

    # ./cloudbackup client backup target test third_backup333 -c client_config.yaml should fail as we're trying a
    # target which should not exist
    def test_cmd_client_backup_target_test2(self):
        result = run_shell_cmd(self.cmd + " client backup target test third_backup333 -c " + self.client_config_file_path)
...
def main():
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(TestBackupTargetTest)
