...
class TestCliNotification2(unittest.TestCase):
    def setUp(self):
...
'''
    ...
    def test_cmd_client_notification_test1(self):
        ...
        matches, email_msg = self.smtp_handler.received_message_matching(r".*Receiving this email proves that the "
                                                                         r"backup server's SMTP\(email\) settings are "
                                                                         r"correct.*")
...
def main():
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(TestCliNotification2)
