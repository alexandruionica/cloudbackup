...
class TestRestoreAzureBlobStorage(unittest.TestCase):
    ...
    def test_restore_by_directory(self):
        ...
        # step 1: configure Azure target and run backup
        self._configure_azure_target(job_name)
        logging.info("Running backup to Azure Blob Storage for directory restore test")
        backup_job_id = self._run_backup_and_wait(job_name)
        ...
        logging.info("Starting directory restore from Azure for path '{}'".format(subdir_to_restore))
