...
import { getConfig, getVersion, listBackupFiles, listBackups, listReports, listRestoreReports, listRestores, resumeRestore, showReport, showRestoreReport, startBackup, startRestore, stopBackup, stopRestore, watchBackup, watchRestore, } from './api.js';
...
function App() {
    ...
    const [restoresByName, setRestoresByName] = useState({});
    ...
    const [restoreReportsFor, setRestoreReportsFor] = useState(null);
    const [restoreStartFor, setRestoreStartFor] = useState(null);
    ...
    const onStopRestore = useCallback(async (name, restoreJobId) => {
        const key = name + '::restore';
        setBusy(b => ({ ...b, [key]: true }));
        try {
            await stopRestore(conn, name, restoreJobId);
            await refresh();
        }
        catch (e) {
            setError(e?.message || String(e));
      ...
      <div class="jobs">
        ${jobs.map(j => html `
          <${JobCard}
            ...
            onWatchRestore=${(rid) => setWatchTarget({ kind: 'restore', name: j.name, jobId: rid, title: j.name + ' (restore)' })}
            onStopRestore=${(rid) => onStopRestore(j.name, rid)}
...
function WatchModal(props) {
    ...
    useEffect(() => {
        ...
        const cancel = handler(conn, target.name, target.jobId, (e) => {
            setEvents(prev => {
...
function RestoreReportsModal(props) {
    ...
    const openDetail = useCallback(async (jobId, state) => {
        setDetailLoading(true);
    ...
    const onResume = useCallback(async (jobId) => {
        if (!pickedTarget) {
    ...
    <div class="modal-backdrop" onClick=${(e) => { if (e.target === e.currentTarget)
      ...
      <div class="modal modal-wide" role="dialog" aria-modal="true">
            ...
            <div class="resume-bar">
              ...
              ${targets.length > 1 ? html `
                <label class="muted" style="text-transform:none;letter-spacing:0">Target:
                  <select value=${pickedTarget} onChange=${(e) => setPickedTarget(e.currentTarget.value)}>
        ...
        : html `<${RestoreReportList}
                ...
                loading=${loading}
                nextToken=${nextToken}
                resumable=${(s) => isResumable(s)}
                onOpen=${(jobId, state) => openDetail(jobId, state)}
                onLoadMore=${() => loadPage(nextToken)} />`}
        </div>
      </div>
    </div>
...
function RestoreStartModal(props) {
    ...
    const [cwd, setCwd] = useState(''); // '' == virtual root
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (candidate)
                    setSourceJobId(candidate.job_id);
            }
            catch (e) {
                if (!stopped)
                    setError(e?.message || String(e));
    ...
    const enterDir = useCallback((path) => {
        setStack(s => [...s, cwd]);
        setCwd(path);
        void loadDir(path);
    }, [cwd, loadDir]);
    ...
    const onSubmit = useCallback(async () => {
        setError(null);
        if (!sourceJobId) {
            setError('Please pick a source backup job to restore from.');
            return;
        }
      ...
      <div class="modal modal-wide" role="dialog" aria-modal="true">
        ...
        <div class="body">
          ...
          <div class="form-row">
            ...
            <select value=${sourceJobId} onChange=${(e) => setSourceJobId(e.currentTarget.value)} disabled=${sourceLoading}>
          ...
          <div class="form-row">
            ...
            <label class="muted" style="text-transform:none;letter-spacing:0;display:flex;align-items:center;gap:6px">
              <input type="checkbox" style="min-width:auto;width:auto" checked=${allFiles}
                     onChange=${(e) => setAllFiles(e.currentTarget.checked)} />
          ...
          <div class="form-row">
            ...
            <select value=${targetName} onChange=${(e) => setTargetName(e.currentTarget.value)}>
          ...
          <div class="form-row">
            ...
            <input type="text" value=${restoreDir} onInput=${(e) => setRestoreDir(e.currentTarget.value)}
