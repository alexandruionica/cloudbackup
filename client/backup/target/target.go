...
const loggingContext = "client.backup.target"
...
// calls the server API in order to request a target (object store) test for a given backup section name
func Test(config clientConfig.Client, jsonOutput bool, jobName string) {
	payload := struct {
	...
	req, err := http.NewRequest("POST", config.Address+ApiPrefix+"/backup/target/test",
